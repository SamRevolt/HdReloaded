/* The meat and potatoes for sending output commands
 ...
 /*
 Changelog:
   - Modified Serial command to Revolv
   - Increased Baud from 57600 to 115200
...
class Device {
    ...
    constructor(path, player, rumbleOnHook, delay, preconnect, quitAfterUnhook) {
        ...
        this.#port = path ? new SerialPort({ path: path, baudRate: 115200, autoOpen: false }) : null;
            ...
            if (this.#preconnectPromise) {
                this.#preconnectPromise.then((err) => {
                    if (err) {
                        ...
                        this.#sendAction("S0&", /* instantaneous */ true, /* important */ true); // all custom feedback
                ...
                this.#port.open((err) => {
                    if (err) {
                        ...
                        that.#sendAction("S0&", /* instantaneous */ true, /* important */ true); // all custom feedback
    ...
    onUnhook = () => {
        ...
        // turn off rumble
        this.#sendAction("F2.0&", /* instantaneous */ true, /* important */ true);
...
//        this.#sendAction("F1." + (maxRumble > 0 ? 1 : 0) + "." + maxRumble, instantaneous, important);
        this.#sendAction("F2." + (maxRumble > 0 ? 1 : 0) + "&");
 ...
 //       this.#sendAction("R0.1." + pause, false, true);
	    this.#sendAction("T0." + hold + "." + pause, false, true);
    };
